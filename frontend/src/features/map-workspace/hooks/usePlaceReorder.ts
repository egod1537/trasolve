...
export function usePlaceReorder({ days, scrollRef, onMovePlace }: Options) {
  ...
  const cancelDrag = useCallback((dayId?: string) => {
    ...
    setDragState(null);
    draggedElementRef.current?.style.removeProperty('--layer-preview-offset');
  ...
  const onPointerDown = useCallback(
    (
      ...
      draggedElementRef.current =
        event.currentTarget.closest<HTMLElement>('.trip-layer-item');
    ...
    const updatePreview = () => {
      ...
      if (target) {
        ...
        const sourceDayIndex = days.findIndex(
          (day) => day.id === next.sourceDayId,
        );
        const targetDayIndex = days.findIndex(
          (day) => day.id === next.targetDayId,
        );
        const parentOffset =
          sourceDayIndex >= 0 &&
          targetDayIndex >= 0 &&
        ...
        draggedElementRef.current?.style.setProperty(
          '--layer-preview-offset',
          `${next.currentY - next.startY - parentOffset}px`,
        );
        if (
  ...
  }, [days, dragState?.placeId, scrollRef]);
